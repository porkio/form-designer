...
export default {
	...
	methods: {
			...
			if (!!this.subFormItemFlag && !this.designState) {
				...
				if (
					(subFormData === undefined ||
						...
						subFormData[this.subFormRowIndex][
							this.field.options.name
						] === undefined) &&
		...
		registerToRefList(oldRefName) {
			if (this.refList !== null && !!this.field.options.name) {
				if (this.subFormItemFlag && !this.designState) {
					...
					if (!!oldRefName) {
						delete this.refList[
							oldRefName + '@row' + this.subFormRowId
						]
					}
					this.refList[
		...
		initOptionItems(keepSelected) {
			...
			if (
				...
				if (
					...
					if (!!keepSelected) {
						this.reloadOptions(
							newOptionItems[this.field.options.name]
						)
					} else {
						this.loadOptions(
							newOptionItems[this.field.options.name]
		...
		handleChangeEvent(value) {
			...
			this.dispatch('VFormRender', 'fieldValidation', [
				this.getPropName(),
			])
		},
